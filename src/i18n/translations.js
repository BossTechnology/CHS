export const LANGUAGES = [
  { code: "EN", label: "English" },
  { code: "ES", label: "Español" },
  { code: "FR", label: "Français" },
  { code: "PT", label: "Português" },
];

export function detectLanguage() {
  const nav = (navigator.language || "en").toLowerCase().slice(0, 2);
  if (nav === "es") return "ES";
  if (nav === "fr") return "FR";
  if (nav === "pt") return "PT";
  return "EN";
}

export const T = {
  EN: {
    tagline: "BUSINESS OBSERVABILITY FRAMEWORK",
    bossLink: "a Boss.Technology",
    pageTitle: "Build a Chass1s.",
    inputInstruction: "ENTER A BUSINESS TYPE or current COMPANY NAME, LOCATION, WEBSITE, OBJECTIVES, CONCEPTS OR ANY COMBINATION — SEPARATED BY COMMAS.",
    deployHint: "⌘ + ENTER TO DEPLOY · OPENS IN NEW TAB",
    startBtn: "START FABRICATING",
    whatIsChs: "WHAT IS CHASS1S?",
    chsDescription: "CHASS1S is the world's first and only Business Observability (BO11Y) Framework — with the power to Fabricate, Tune, and even Drive any new or existing business, from an ice cream shop to an airline. For the first time, technology and business are unified into a single powerful vehicle — where every decision, every change, and every insight cascades with full awareness of the whole. Through Business Observability, powered by Applied AI, CHASS1S turns the complexity of running a business into something observable, measurable, and actionable. The modern Framework for Fast Companies and Real-Time Businesses.",
    chsCreditBoss: { prefix: "CHASS1S is brought to you by ", name: "Boss.Technology", suffix: " — Latin American based Premium Business Observability Builder, Tuner, and Brand." },
    chsCreditFederico: { prefix: "Created by ", name: "Federico Lara", suffix: " — Business Observability thought leader, author of Six Steps to Achieve Business Observability, creator of Metric Monetization, and former executive at New Relic and Dynatrace." },
    selectDepth: "SELECT CHASSIS DEPTH",
    tabs: { intro: "INTRO", introSub: "Business Overview", addis: "ADDIS", addisSub: "Technology Layer", blips: "BLIPS", blipsSub: "Business Layer", kbrs: "KBRs", kbrsSub: "Key Business Results" },
    expandAll: "EXPAND ALL", collapseAll: "COLLAPSE ALL",
    sections: "SECTIONS", items: "ITEMS",
    businessProfile: "BUSINESS PROFILE", whyChassis: "CHASSIS", keyValuePoints: "KEY VALUE POINTS", frameworkApplied: "CHASSIS APPLIED",
    addisDesc: "Maps every technology tool, platform, system, and infrastructure component this business depends on.",
    blipsDesc: "Maps every business function and the observable tools and data sources that connect technology performance to business outcomes.",
    kbrTitle: "Key Business Results", kbrsAcross: "KBRs ACROSS", businessAreas: "BUSINESS AREAS",
    metric: "METRIC", target: "TARGET",
    newChassis: "NEW CHASSIS",
    loadingTitle: "PROCESSING",
    loadingSteps: ["ANALYZING BUSINESS TYPE...", "MAPPING ADDIS TECHNOLOGY LAYER...", "BUILDING BLIPS BUSINESS CHASSIS...", "GENERATING KEY BUSINESS RESULTS...", "FABRICATING CHASSIS..."],
    fabricatingPrefix: "FABRICATING",
    charmingPhrases: [
      "This one's moving fast — brace yourself.",
      "Still here? Good. So are we. This is going well.",
      "Go ahead and stretch a little. We'll be ready when you are.",
      "This is taking a moment. A good moment — like a fine espresso kind of moment.",
      "While you're waiting, go hug a pet... it doesn't have to be your own.",
      "Could be a few minutes — go ahead and order that overly complicated beverage you love.",
      "We're building something custom, specific, yours. Good things take time.",
      "Still going. This is our most thorough work. Make yourself a snack. You've earned it.",
    ],
    errorTitle: "Something went wrong.", tryAgain: "TRY AGAIN",
    tableHeaders: ["Item", "Type", "Description", "In/Ex", "Environment", "Data"],
    tableHeadersBlips: ["Item", "Source", "Type", "Description", "In/Ex", "Environment", "Data"],
    noItems: "No items generated.",
    tiers: [
      { id: "compact", label: "Compact", description: "Essential overview. Core ADDIS & BLIPS items with focused KBRs. Ideal for quick assessments." },
      { id: "midsize", label: "Mid-Size", description: "Enhanced depth with richer context. More items, fuller descriptions, and expanded real-world factors." },
      { id: "executive", label: "Executive", description: "Comprehensive analysis with detailed context, real-world coverage, and rich KBR targets with industry benchmarks." },
      { id: "luxury", label: "Luxury", description: "Enterprise-grade intelligence. Maximum depth, narrative impact analysis, risk register, quick wins, and full industry context." },
    ],
    examples: [
      "Häagen-Dazs, UK Regent Street, https://www.haagen-dazs.co.uk/, Increase sales by 20% during the winter months.",
      "Motorcycle Shop, Medellín Colombia Laureles, Add Rentals and custom tuning new services, $2,000 USD budget.",
      "Mercado Libre, https://www.mercadolibre.com/",
      "Boutique Hotel, Cartagena Colombia, Expand bookings for international travelers, Launch loyalty program.",
      "Tesla, https://www.tesla.com/, Optimize service center operations, Reduce wait times by 30%.",
      "Independent Pharmacy, Miami Florida, Compete with CVS and Walgreens, Add delivery and telehealth services.",
      "CrossFit Gym, Brooklyn New York, Grow membership base, Launch online training program, $5,000 monthly budget.",
      "Family Law Firm, Mexico City, Attract high-net-worth clients, Build digital presence and referral strategy.",
    ],
    beyondProfit: "BEYOND PROFIT",
    beyondProfitSub: "Optional",
    beyondProfitDesc: "Select any initiatives relevant to this business. Results will appear as a dedicated tab.",
    beyondProfitTab: "Beyond Profit",
    beyondProfitTabSub: "Legal, Social & Regulatory Insights",
    bpOptions: { CSR: "CSR", ESG: "ESG", DEI: "DEI", TBL: "TBL", Sustainability: "SUS" },
    bpDefinitions: {
      CSR: "CSR — Corporate Social Responsibility",
      ESG: "ESG — Environmental, Social & Governance",
      DEI: "DEI — Diversity, Equity & Inclusion",
      TBL: "TBL — Triple Bottom Line",
      Sustainability: "SUS — Sustainability",
    },
    useTokens: "USE",
    tokensPer: "1 TOKEN PER",
    bpLabels: { legal: "LEGAL", social: "SOCIAL", regulatory: "REGULATORY", news: "NEWS & DEVELOPMENTS", suggestions: "SUGGESTIONS" },
    bp: { analyzing: "ANALYZING", gatheringContext: "GATHERING LEGAL & REGULATORY CONTEXT...", generatingInsights: "GENERATING INSIGHTS & SUGGESTIONS...", errorMsg: "Could not generate Beyond Profit insights." },
    pdfControlled: "CONTROLLED",
    pdfUncontrolled: "UNCONTROLLED",
    errorEmailNotConfirmed: "Please confirm your email address before generating. Check your inbox.",
    auth: {
      signInTitle:    "Sign In to CHASS1S",
      signUpTitle:    "Create Your Account",
      resetTitle:     "Reset Your Password",
      signInTab:      "SIGN IN",
      signUpTab:      "CREATE ACCOUNT",
      forgotTab:      "RESET PASSWORD",
      emailLabel:     "EMAIL ADDRESS",
      passwordLabel:  "PASSWORD",
      redirecting:    "REDIRECTING...",
      processing:     "PROCESSING...",
      sending:        "SENDING...",
      sendReset:      "SEND RESET EMAIL",
      backToSignIn:   "Back to Sign In",
      forgotLink:     "Forgot password?",
      signUpSuccess:  "Account created! Check your email to confirm.",
      resetSuccess:   "Password reset email sent. Check your inbox.",
      freeTokens:     "New accounts receive 10 FREE TOKENS — no credit card required.",
      orContinueWith: "OR CONTINUE WITH",
    },
    // ── Account Menu ──
    menuSignedInAs: "SIGNED IN AS",
    menuWorkspace: "WORKSPACE",
    menuPersonal: "Personal",
    menuAddWorkspace: "+ Work Space",
    menuChassis: "CHASSIS",
    menuTokenBalance: "TOKEN BALANCE",
    menuBuyMore: "+ Buy More",
    menuLanguage: "LANGUAGE",
    menuSignOut: "SIGN OUT",
    // ── Settings & Support ──
    menuSettings: "Settings",
    menuSupport: "Support",
    settingsTitle: "Account Settings",
    settingsSectionProfile: "PROFILE INFO",
    settingsSave: "SAVE CHANGES",
    settingsSaving: "SAVING...",
    settingsSaved: "✓ Saved",
    settingsSectionSecurity: "SECURITY",
    settingsNewEmail: "NEW EMAIL ADDRESS",
    settingsUpdateEmail: "UPDATE EMAIL",
    settingsCurrentPassword: "CURRENT PASSWORD",
    settingsNewPassword: "NEW PASSWORD",
    settingsConfirmPassword: "CONFIRM NEW PASSWORD",
    settingsUpdatePassword: "UPDATE PASSWORD",
    settingsOAuthNoPassword: "Password login is not available for your account. You signed in with a social provider.",
    settingsSectionConnected: "CONNECTED ACCOUNTS",
    settingsSectionDanger: "CLOSE ACCOUNT",
    settingsDangerWarning: "Your account will be scheduled for deletion in 30 days. You can reactivate by signing in before that date.",
    settingsDangerPrompt: "Type DELETE to confirm",
    settingsDeactivateBtn: "DEACTIVATE ACCOUNT",
    settingsDeactivated: "Account deactivated. You will be signed out.",
    settingsDangerShow: "Show options",
    supportTitle: "Contact Support",
    supportCategoryLabel: "ISSUE CATEGORY",
    supportCategories: ["Billing & Tokens", "Technical Issue", "Account Access", "Generation Quality", "Feature Request", "Other"],
    supportDescriptionLabel: "DESCRIPTION",
    supportDescriptionPlaceholder: "Describe your issue in detail (minimum 20 characters)...",
    supportAttachLabel: "ATTACHMENT (optional)",
    supportChooseFile: "CHOOSE FILE",
    supportFileSizeError: "File must be under 5MB",
    supportFileTypeError: "Only images and PDF files are accepted",
    supportSubmit: "SUBMIT TICKET",
    supportSubmitting: "SUBMITTING...",
    supportSuccessTitle: "Ticket Submitted",
    supportSuccessMsg: "We'll get back to you within 24–48 hours. Your ticket number is:",
    supportClose: "CLOSE",
    modeSelector: {
      fabricate: "Fabricate",
      tune: "Tune",
      drive: "Drive",
      go: "GO",
      comingSoon: "Coming Soon",
    },
    reFabricate: {
      title: "Re-Fabricate Chassis",
      contextNote: "Your current Chassis context will be used to build an even richer result.",
      originalInput: "ORIGINAL INPUT",
      editHint: "Edit or expand your input before re-fabricating.",
      selectType: "SELECT CHASSIS TYPE",
      confirm: "RE-FABRICATE",
    },
    chassisHistory: {
      title: "Chassis History",
      actionFabricated: "Fabricated",
      actionReFabricated: "Re-Fabricated",
      actionTuned: "Tuned",
      chassisType: "CHASSIS TYPE",
      beyondProfit: "BEYOND PROFIT",
      none: "None",
      noHistory: "No history yet.",
    },
  },
  ES: {
    tagline: "BUSINESS OBSERVABILITY FRAMEWORK",
    bossLink: "una Boss.Technology",
    pageTitle: "Construir un Chass1s.",
    inputInstruction: "INGRESE UN TIPO DE NEGOCIO o NOMBRE DE EMPRESA ACTUAL, UBICACIÓN, SITIO WEB, OBJETIVOS, CONCEPTOS O CUALQUIER COMBINACIÓN — SEPARADOS POR COMAS.",
    deployHint: "⌘ + ENTER PARA DESPLEGAR · SE ABRE EN NUEVA PESTAÑA",
    startBtn: "INICIAR FABRICACIÓN",
    whatIsChs: "¿QUÉ ES CHASS1S?",
    chsDescription: "CHASS1S es el primer y único Business Observability (BO11Y) Framework del mundo — con el poder de Fabricar, Afinar e incluso Conducir cualquier negocio nuevo o existente, desde una heladería hasta una aerolínea. Por primera vez, la tecnología y el negocio se unifican en un solo vehículo poderoso — donde cada decisión, cada cambio y cada insight resuena con plena consciencia del conjunto. A través de Business Observability, impulsada por Applied AI, CHASS1S convierte la complejidad de dirigir un negocio en algo observable, medible y accionable. El Framework moderno para Empresas Rápidas y Negocios en Tiempo Real.",
    chsCreditBoss: { prefix: "CHASS1S es traído a ti por ", name: "Boss.Technology", suffix: " — Premium Business Observability Builder, Tuner y Brand con base en Latinoamérica." },
    chsCreditFederico: { prefix: "Creado por ", name: "Federico Lara", suffix: " — líder de pensamiento en Business Observability, autor de Six Steps to Achieve Business Observability, creador de Metric Monetization, y ex ejecutivo en New Relic y Dynatrace." },
    selectDepth: "SELECCIONAR PROFUNDIDAD DEL CHASSIS",
    tabs: { intro: "INTRO", introSub: "Perfil del Negocio", addis: "ADDIS", addisSub: "Capa Tecnológica", blips: "BLIPS", blipsSub: "Capa Empresarial", kbrs: "KBRs", kbrsSub: "Key Business Results" },
    expandAll: "EXPANDIR TODO", collapseAll: "COLAPSAR TODO",
    sections: "SECCIONES", items: "ELEMENTOS",
    businessProfile: "PERFIL DEL NEGOCIO", whyChassis: "CHASSIS", keyValuePoints: "PUNTOS DE VALOR CLAVE", frameworkApplied: "CHASSIS APLICADO",
    addisDesc: "Mapea cada herramienta tecnológica, plataforma, sistema y componente de infraestructura del que depende este negocio.",
    blipsDesc: "Mapea cada función empresarial y las fuentes de datos observables que conectan el rendimiento tecnológico con los resultados del negocio.",
    kbrTitle: "Key Business Results", kbrsAcross: "KBRs ACROSS", businessAreas: "BUSINESS AREAS",
    metric: "MÉTRICA", target: "OBJETIVO",
    newChassis: "NUEVO CHASSIS",
    loadingTitle: "PROCESANDO",
    loadingSteps: ["ANALIZANDO TIPO DE NEGOCIO...", "MAPEANDO CAPA TECNOLÓGICA ADDIS...", "CONSTRUYENDO CHASSIS EMPRESARIAL BLIPS...", "GENERANDO RESULTADOS CLAVE DEL NEGOCIO...", "FABRICANDO CHASSIS..."],
    fabricatingPrefix: "FABRICANDO",
    charmingPhrases: [
      "Esto va rápido — prepárate.",
      "¿Sigues aquí? Bien. Nosotros también. Todo va bien.",
      "Estírate un poco. Estaremos listos cuando tú lo estés.",
      "Esto toma un momento. Un buen momento — como el de un buen café expreso.",
      "Mientras esperas, ve a abrazar a una mascota... no tiene que ser la tuya.",
      "Puede tomar unos minutos — ve y pide esa bebida complicada que tanto te gusta.",
      "Estamos construyendo algo a tu medida, específico, tuyo. Las cosas buenas toman tiempo.",
      "Seguimos. Este es nuestro trabajo más profundo. Ve por algo de comer. Te lo mereces.",
    ],
    errorTitle: "Algo salió mal.", tryAgain: "INTENTAR DE NUEVO",
    tableHeaders: ["Elemento", "Tipo", "Descripción", "Int/Ext", "Entorno", "Datos"],
    tableHeadersBlips: ["Elemento", "Fuente", "Tipo", "Descripción", "Int/Ext", "Entorno", "Datos"],
    noItems: "No se generaron elementos.",
    tiers: [
      { id: "compact", label: "Compacto", description: "Visión esencial. Elementos básicos de ADDIS y BLIPS con KBRs enfocados. Ideal para evaluaciones rápidas." },
      { id: "midsize", label: "Mediano", description: "Mayor profundidad con más contexto. Más elementos, descripciones completas y factores del mundo real ampliados." },
      { id: "executive", label: "Ejecutivo", description: "Análisis exhaustivo con contexto detallado, cobertura del mundo real y KBRs enriquecidos con benchmarks de la industria." },
      { id: "luxury", label: "Lujo", description: "Inteligencia empresarial de nivel superior. Máxima profundidad, análisis de impacto narrativo, registro de riesgos y victorias rápidas." },
    ],
    examples: [
      "Häagen-Dazs, Regent Street Londres UK, https://www.haagen-dazs.co.uk/, Aumentar ventas un 20% en meses de invierno.",
      "Taller de motos, Medellín Colombia Laureles, Añadir servicios de renta y tuning personalizado, Presupuesto $2,000 USD.",
      "Mercado Libre, https://www.mercadolibre.com/",
      "Hotel Boutique, Cartagena Colombia, Expandir reservas de viajeros internacionales, Lanzar programa de fidelización.",
      "Tesla, https://www.tesla.com/, Optimizar operaciones de centros de servicio, Reducir tiempos de espera un 30%.",
      "Farmacia Independiente, Miami Florida, Competir con grandes cadenas, Lanzar servicio de domicilios y telemedicina.",
      "Gimnasio CrossFit, Brooklyn Nueva York, Crecer base de miembros, Lanzar programa de entrenamiento en línea.",
      "Firma de Derecho Familiar, Ciudad de México, Atraer clientes de alto valor, Estrategia de presencia digital.",
    ],
    beyondProfit: "BEYOND PROFIT",
    beyondProfitSub: "Opcional",
    beyondProfitDesc: "Seleccione las iniciativas relevantes para este negocio. Los resultados aparecerán como una pestaña dedicada.",
    beyondProfitTab: "Beyond Profit",
    beyondProfitTabSub: "Perspectivas Legales, Sociales y Regulatorias",
    bpOptions: { CSR: "RSE", ESG: "ESG", DEI: "DEI", TBL: "TBL", Sustainability: "SUS" },
    bpDefinitions: {
      CSR: "RSE — Responsabilidad Social Empresarial",
      ESG: "ESG — Ambiental, Social y Gobernanza",
      DEI: "DEI — Diversidad, Equidad e Inclusión",
      TBL: "TBL — Triple Línea de Base",
      Sustainability: "SUS — Sostenibilidad",
    },
    useTokens: "USAR",
    tokensPer: "1 TOKEN POR",
    bpLabels: { legal: "LEGAL", social: "SOCIAL", regulatory: "REGULATORIO", news: "NOTICIAS Y DESARROLLOS", suggestions: "SUGERENCIAS" },
    bp: { analyzing: "ANALIZANDO", gatheringContext: "RECOPILANDO CONTEXTO LEGAL Y REGULATORIO...", generatingInsights: "GENERANDO INSIGHTS Y SUGERENCIAS...", errorMsg: "No se pudieron generar los insights de Beyond Profit." },
    pdfControlled: "CONTROLADO",
    pdfUncontrolled: "NO CONTROLADO",
    errorEmailNotConfirmed: "Por favor confirma tu dirección de email antes de generar. Revisa tu bandeja de entrada.",
    auth: {
      signInTitle:    "Iniciar Sesión en CHASS1S",
      signUpTitle:    "Crear Tu Cuenta",
      resetTitle:     "Restablecer Tu Contraseña",
      signInTab:      "INICIAR SESIÓN",
      signUpTab:      "CREAR CUENTA",
      forgotTab:      "RESTABLECER",
      emailLabel:     "CORREO ELECTRÓNICO",
      passwordLabel:  "CONTRASEÑA",
      redirecting:    "REDIRIGIENDO...",
      processing:     "PROCESANDO...",
      sending:        "ENVIANDO...",
      sendReset:      "ENVIAR EMAIL DE RESTABLECIMIENTO",
      backToSignIn:   "Volver al Inicio de Sesión",
      forgotLink:     "¿Olvidaste tu contraseña?",
      signUpSuccess:  "¡Cuenta creada! Revisa tu email para confirmar.",
      resetSuccess:   "Email de restablecimiento enviado. Revisa tu bandeja.",
      freeTokens:     "Las cuentas nuevas reciben 10 TOKENS GRATIS — sin tarjeta de crédito.",
      orContinueWith: "O CONTINUAR CON",
    },
    // ── Account Menu ──
    menuSignedInAs: "SESIÓN COMO",
    menuWorkspace: "ESPACIO DE TRABAJO",
    menuPersonal: "Personal",
    menuAddWorkspace: "+ Espacio de Trabajo",
    menuChassis: "CHASSIS",
    menuTokenBalance: "SALDO DE TOKENS",
    menuBuyMore: "+ Comprar Más",
    menuLanguage: "IDIOMA",
    menuSignOut: "CERRAR SESIÓN",
    // ── Settings & Support ──
    menuSettings: "Configuración",
    menuSupport: "Soporte",
    settingsTitle: "Configuración de Cuenta",
    settingsSectionProfile: "INFORMACIÓN DE PERFIL",
    settingsSave: "GUARDAR CAMBIOS",
    settingsSaving: "GUARDANDO...",
    settingsSaved: "✓ Guardado",
    settingsSectionSecurity: "SEGURIDAD",
    settingsNewEmail: "NUEVO CORREO ELECTRÓNICO",
    settingsUpdateEmail: "ACTUALIZAR EMAIL",
    settingsCurrentPassword: "CONTRASEÑA ACTUAL",
    settingsNewPassword: "NUEVA CONTRASEÑA",
    settingsConfirmPassword: "CONFIRMAR NUEVA CONTRASEÑA",
    settingsUpdatePassword: "ACTUALIZAR CONTRASEÑA",
    settingsOAuthNoPassword: "El inicio de sesión con contraseña no está disponible para tu cuenta. Iniciaste sesión con un proveedor social.",
    settingsSectionConnected: "CUENTAS CONECTADAS",
    settingsSectionDanger: "CERRAR CUENTA",
    settingsDangerWarning: "Tu cuenta quedará programada para eliminación en 30 días. Puedes reactivarla iniciando sesión antes de esa fecha.",
    settingsDangerPrompt: "Escribe DELETE para confirmar",
    settingsDeactivateBtn: "DESACTIVAR CUENTA",
    settingsDeactivated: "Cuenta desactivada. Serás desconectado.",
    settingsDangerShow: "Mostrar opciones",
    supportTitle: "Contactar Soporte",
    supportCategoryLabel: "CATEGORÍA DEL PROBLEMA",
    supportCategories: ["Facturación y Tokens", "Problema Técnico", "Acceso a Cuenta", "Calidad de Generación", "Solicitud de Función", "Otro"],
    supportDescriptionLabel: "DESCRIPCIÓN",
    supportDescriptionPlaceholder: "Describe tu problema en detalle (mínimo 20 caracteres)...",
    supportAttachLabel: "ADJUNTO (opcional)",
    supportChooseFile: "ELEGIR ARCHIVO",
    supportFileSizeError: "El archivo debe ser menor de 5MB",
    supportFileTypeError: "Solo se aceptan imágenes y archivos PDF",
    supportSubmit: "ENVIAR TICKET",
    supportSubmitting: "ENVIANDO...",
    supportSuccessTitle: "Ticket Enviado",
    supportSuccessMsg: "Te responderemos en 24–48 horas. Tu número de ticket es:",
    supportClose: "CERRAR",
    modeSelector: {
      fabricate: "Fabricar",
      tune: "Afinar",
      drive: "Conducir",
      go: "IR",
      comingSoon: "Próximamente",
    },
    reFabricate: {
      title: "Re-Fabricar Chass1s",
      contextNote: "Tu Chass1s actual será usado para construir un resultado aún más rico.",
      originalInput: "ENTRADA ORIGINAL",
      editHint: "Edita o amplía tu entrada antes de re-fabricar.",
      selectType: "SELECCIONAR TIPO",
      confirm: "RE-FABRICAR",
    },
    chassisHistory: {
      title: "Historial del Chass1s",
      actionFabricated: "Fabricado",
      actionReFabricated: "Re-Fabricado",
      actionTuned: "Afinado",
      chassisType: "TIPO DE CHASS1S",
      beyondProfit: "BEYOND PROFIT",
      none: "Ninguno",
      noHistory: "Sin historial aún.",
    },
  },
  FR: {
    tagline: "BUSINESS OBSERVABILITY FRAMEWORK",
    bossLink: "une Boss.Technology",
    pageTitle: "Construire un Chass1s.",
    inputInstruction: "SAISISSEZ UN TYPE D'ENTREPRISE ou NOM D'ENTREPRISE ACTUEL, LIEU, SITE WEB, OBJECTIFS, CONCEPTS OU TOUTE COMBINAISON — SÉPARÉS PAR DES VIRGULES.",
    deployHint: "⌘ + ENTRÉE POUR DÉPLOYER · OUVRE DANS UN NOUVEL ONGLET",
    startBtn: "COMMENCER LA FABRICATION",
    whatIsChs: "QU'EST-CE QUE CHASS1S?",
    chsDescription: "CHASS1S est le premier et unique Business Observability (BO11Y) Framework au monde — avec le pouvoir de Fabriquer, Affiner et même Piloter toute entreprise nouvelle ou existante, d'un glacier à une compagnie aérienne. Pour la première fois, la technologie et l'entreprise sont unifiées en un seul véhicule puissant — où chaque décision, chaque changement et chaque insight se répercute avec une pleine conscience de l'ensemble. À travers Business Observability, propulsée par Applied AI, CHASS1S transforme la complexité de gérer une entreprise en quelque chose d'observable, mesurable et actionnable. Le Framework moderne pour les Entreprises Rapides et les Businesses en Temps Réel.",
    chsCreditBoss: { prefix: "CHASS1S vous est proposé par ", name: "Boss.Technology", suffix: " — Premium Business Observability Builder, Tuner et Brand basé en Amérique Latine." },
    chsCreditFederico: { prefix: "Créé par ", name: "Federico Lara", suffix: " — leader d'opinion en Business Observability, auteur de Six Steps to Achieve Business Observability, créateur de Metric Monetization, et ancien dirigeant chez New Relic et Dynatrace." },
    selectDepth: "SÉLECTIONNER LA PROFONDEUR DU CHASSIS",
    tabs: { intro: "INTRO", introSub: "Profil de l'Entreprise", addis: "ADDIS", addisSub: "Couche Technologique", blips: "BLIPS", blipsSub: "Couche Métier", kbrs: "KBRs", kbrsSub: "Key Business Results" },
    expandAll: "TOUT DÉVELOPPER", collapseAll: "TOUT RÉDUIRE",
    sections: "SECTIONS", items: "ÉLÉMENTS",
    businessProfile: "PROFIL DE L'ENTREPRISE", whyChassis: "CHASSIS", keyValuePoints: "POINTS DE VALEUR CLÉS", frameworkApplied: "CHASSIS APPLIQUÉ",
    addisDesc: "Cartographie chaque outil technologique, plateforme, système et composant d'infrastructure dont dépend cette entreprise.",
    blipsDesc: "Cartographie chaque fonction métier et les sources de données observables qui relient la performance technologique aux résultats commerciaux.",
    kbrTitle: "Key Business Results", kbrsAcross: "KBRs ACROSS", businessAreas: "BUSINESS AREAS",
    metric: "MÉTRIQUE", target: "OBJECTIF",
    newChassis: "NOUVEAU CHASSIS",
    loadingTitle: "TRAITEMENT EN COURS",
    loadingSteps: ["ANALYSE DU TYPE D'ENTREPRISE...", "CARTOGRAPHIE DE LA COUCHE ADDIS...", "CONSTRUCTION DU CHASSIS BLIPS...", "GÉNÉRATION DES RÉSULTATS CLÉS...", "FABRICATION DU CHASSIS..."],
    fabricatingPrefix: "FABRICATION",
    charmingPhrases: [
      "Ça avance vite — préparez-vous.",
      "Toujours là ? Bien. Nous aussi. Tout se passe bien.",
      "Étirez-vous un peu. Nous serons prêts quand vous l'êtes.",
      "Cela prend un moment. Un bon moment — comme celui d'un bon espresso.",
      "En attendant, allez câliner un animal... il ne doit pas être le vôtre.",
      "Cela peut prendre quelques minutes — commandez cette boisson trop compliquée que vous adorez.",
      "Nous construisons quelque chose sur mesure, précis, qui vous appartient. Les bonnes choses prennent du temps.",
      "On continue. C'est notre travail le plus approfondi. Allez vous chercher une collation. Vous le méritez.",
    ],
    errorTitle: "Une erreur s'est produite.", tryAgain: "RÉESSAYER",
    tableHeaders: ["Élément", "Type", "Description", "Int/Ext", "Environnement", "Données"],
    tableHeadersBlips: ["Élément", "Source", "Type", "Description", "Int/Ext", "Environnement", "Données"],
    noItems: "Aucun élément généré.",
    tiers: [
      { id: "compact", label: "Compact", description: "Vue d'ensemble essentielle. Éléments ADDIS et BLIPS principaux avec des KBRs ciblés. Idéal pour les évaluations rapides." },
      { id: "midsize", label: "Moyen", description: "Profondeur enrichie avec plus de contexte. Plus d'éléments, descriptions complètes et facteurs réels élargis." },
      { id: "executive", label: "Exécutif", description: "Analyse complète avec contexte détaillé, couverture approfondie et KBRs enrichis avec des benchmarks sectoriels." },
      { id: "luxury", label: "Luxe", description: "Intelligence de niveau entreprise. Profondeur maximale, analyse narrative, registre des risques et gains rapides." },
    ],
    examples: [
      "Häagen-Dazs, Regent Street Londres, https://www.haagen-dazs.co.uk/, Augmenter les ventes de 20% durant les mois d'hiver.",
      "Atelier de motos, Medellín Colombie Laureles, Ajouter des services de location et tuning sur mesure, Budget 2 000 USD.",
      "Mercado Libre, https://www.mercadolibre.com/",
      "Hôtel Boutique, Carthagène Colombie, Développer les réservations de voyageurs internationaux, Lancer un programme de fidélité.",
      "Tesla, https://www.tesla.com/, Optimiser les opérations des centres de service, Réduire les délais d'attente de 30%.",
      "Pharmacie Indépendante, Miami Floride, Concurrencer les grandes chaînes, Lancer la livraison et la télémédecine.",
      "Salle CrossFit, Brooklyn New York, Développer la base de membres, Lancer un programme d'entraînement en ligne.",
      "Cabinet de Droit de la Famille, Mexico, Attirer des clients à haute valeur, Stratégie de présence digitale.",
    ],
    beyondProfit: "BEYOND PROFIT",
    beyondProfitSub: "Optionnel",
    beyondProfitDesc: "Sélectionnez les initiatives pertinentes pour cette entreprise. Les résultats apparaîtront dans un onglet dédié.",
    beyondProfitTab: "Beyond Profit",
    beyondProfitTabSub: "Perspectives Juridiques, Sociales et Réglementaires",
    bpOptions: { CSR: "RSE", ESG: "ESG", DEI: "DEI", TBL: "TBL", Sustainability: "SUS" },
    bpDefinitions: {
      CSR: "RSE — Responsabilité Sociale des Entreprises",
      ESG: "ESG — Environnement, Social et Gouvernance",
      DEI: "DEI — Diversité, Équité et Inclusion",
      TBL: "TBL — Triple Bilan",
      Sustainability: "SUS — Durabilité",
    },
    useTokens: "UTILISER",
    tokensPer: "1 TOKEN PAR",
    bpLabels: { legal: "JURIDIQUE", social: "SOCIAL", regulatory: "RÉGLEMENTAIRE", news: "ACTUALITÉS", suggestions: "SUGGESTIONS" },
    bp: { analyzing: "ANALYSE EN COURS", gatheringContext: "COLLECTE DU CONTEXTE JURIDIQUE ET RÉGLEMENTAIRE...", generatingInsights: "GÉNÉRATION DES INSIGHTS ET SUGGESTIONS...", errorMsg: "Impossible de générer les insights Beyond Profit." },
    pdfControlled: "CONTRÔLÉ",
    pdfUncontrolled: "NON CONTRÔLÉ",
    errorEmailNotConfirmed: "Veuillez confirmer votre adresse email avant de générer. Vérifiez votre boîte de réception.",
    auth: {
      signInTitle:    "Connexion à CHASS1S",
      signUpTitle:    "Créer Votre Compte",
      resetTitle:     "Réinitialiser Votre Mot de Passe",
      signInTab:      "CONNEXION",
      signUpTab:      "CRÉER UN COMPTE",
      forgotTab:      "RÉINITIALISER",
      emailLabel:     "ADRESSE EMAIL",
      passwordLabel:  "MOT DE PASSE",
      redirecting:    "REDIRECTION...",
      processing:     "TRAITEMENT...",
      sending:        "ENVOI...",
      sendReset:      "ENVOYER L'EMAIL DE RÉINITIALISATION",
      backToSignIn:   "Retour à la Connexion",
      forgotLink:     "Mot de passe oublié ?",
      signUpSuccess:  "Compte créé ! Vérifiez votre email pour confirmer.",
      resetSuccess:   "Email de réinitialisation envoyé. Vérifiez votre boîte.",
      freeTokens:     "Les nouveaux comptes reçoivent 10 TOKENS GRATUITS — sans carte bancaire.",
      orContinueWith: "OU CONTINUER AVEC",
    },
    // ── Account Menu ──
    menuSignedInAs: "CONNECTÉ EN TANT QUE",
    menuWorkspace: "ESPACE DE TRAVAIL",
    menuPersonal: "Personnel",
    menuAddWorkspace: "+ Espace de Travail",
    menuChassis: "CHASSIS",
    menuTokenBalance: "SOLDE DE TOKENS",
    menuBuyMore: "+ Acheter Plus",
    menuLanguage: "LANGUE",
    menuSignOut: "SE DÉCONNECTER",
    // ── Settings & Support ──
    menuSettings: "Paramètres",
    menuSupport: "Support",
    settingsTitle: "Paramètres du Compte",
    settingsSectionProfile: "INFORMATIONS DE PROFIL",
    settingsSave: "ENREGISTRER",
    settingsSaving: "ENREGISTREMENT...",
    settingsSaved: "✓ Enregistré",
    settingsSectionSecurity: "SÉCURITÉ",
    settingsNewEmail: "NOUVEL EMAIL",
    settingsUpdateEmail: "METTRE À JOUR L'EMAIL",
    settingsCurrentPassword: "MOT DE PASSE ACTUEL",
    settingsNewPassword: "NOUVEAU MOT DE PASSE",
    settingsConfirmPassword: "CONFIRMER LE MOT DE PASSE",
    settingsUpdatePassword: "METTRE À JOUR LE MOT DE PASSE",
    settingsOAuthNoPassword: "La connexion par mot de passe n'est pas disponible pour votre compte. Vous vous êtes connecté via un fournisseur social.",
    settingsSectionConnected: "COMPTES CONNECTÉS",
    settingsSectionDanger: "FERMER LE COMPTE",
    settingsDangerWarning: "Votre compte sera programmé pour suppression dans 30 jours. Vous pouvez le réactiver en vous connectant avant cette date.",
    settingsDangerPrompt: "Tapez DELETE pour confirmer",
    settingsDeactivateBtn: "DÉSACTIVER LE COMPTE",
    settingsDeactivated: "Compte désactivé. Vous allez être déconnecté.",
    settingsDangerShow: "Afficher les options",
    supportTitle: "Contacter le Support",
    supportCategoryLabel: "CATÉGORIE DU PROBLÈME",
    supportCategories: ["Facturation & Tokens", "Problème Technique", "Accès au Compte", "Qualité de Génération", "Demande de Fonctionnalité", "Autre"],
    supportDescriptionLabel: "DESCRIPTION",
    supportDescriptionPlaceholder: "Décrivez votre problème en détail (minimum 20 caractères)...",
    supportAttachLabel: "PIÈCE JOINTE (optionnel)",
    supportChooseFile: "CHOISIR FICHIER",
    supportFileSizeError: "Le fichier doit faire moins de 5 Mo",
    supportFileTypeError: "Seules les images et les fichiers PDF sont acceptés",
    supportSubmit: "SOUMETTRE LE TICKET",
    supportSubmitting: "ENVOI EN COURS...",
    supportSuccessTitle: "Ticket Soumis",
    supportSuccessMsg: "Nous vous répondrons dans 24–48 heures. Votre numéro de ticket est :",
    supportClose: "FERMER",
    modeSelector: {
      fabricate: "Fabriquer",
      tune: "Régler",
      drive: "Conduire",
      go: "GO",
      comingSoon: "Bientôt",
    },
    reFabricate: {
      title: "Re-Fabriquer le Chass1s",
      contextNote: "Votre Chass1s actuel sera utilisé pour produire un résultat encore plus riche.",
      originalInput: "SAISIE ORIGINALE",
      editHint: "Modifiez ou complétez votre saisie avant de re-fabriquer.",
      selectType: "SÉLECTIONNER TYPE",
      confirm: "RE-FABRIQUER",
    },
    chassisHistory: {
      title: "Historique du Chass1s",
      actionFabricated: "Fabriqué",
      actionReFabricated: "Re-Fabriqué",
      actionTuned: "Réglé",
      chassisType: "TYPE DE CHASS1S",
      beyondProfit: "BEYOND PROFIT",
      none: "Aucun",
      noHistory: "Aucun historique.",
    },
  },
  PT: {
    tagline: "BUSINESS OBSERVABILITY FRAMEWORK",
    bossLink: "uma Boss.Technology",
    pageTitle: "Construa um Chass1s.",
    inputInstruction: "INSIRA UM TIPO DE NEGÓCIO ou NOME DE EMPRESA ATUAL, LOCALIZAÇÃO, SITE, OBJETIVOS, CONCEITOS OU QUALQUER COMBINAÇÃO — SEPARADOS POR VÍRGULAS.",
    deployHint: "⌘ + ENTER PARA IMPLANTAR · ABRE EM NOVA ABA",
    startBtn: "INICIAR FABRICAÇÃO",
    whatIsChs: "O QUE É CHASS1S?",
    chsDescription: "CHASS1S é o primeiro e único Business Observability (BO11Y) Framework do mundo — com o poder de Fabricar, Afinar e até Conduzir qualquer negócio novo ou existente, de uma sorveteria a uma companhia aérea. Pela primeira vez, tecnologia e negócio são unificados em um único veículo poderoso — onde cada decisão, cada mudança e cada insight reverbera com plena consciência do todo. Através de Business Observability, impulsionado por Applied AI, CHASS1S transforma a complexidade de gerir um negócio em algo observável, mensurável e acionável. O Framework moderno para Empresas Rápidas e Negócios em Tempo Real.",
    chsCreditBoss: { prefix: "CHASS1S é trazido a você pela ", name: "Boss.Technology", suffix: " — Premium Business Observability Builder, Tuner e Brand com base na América Latina." },
    chsCreditFederico: { prefix: "Criado por ", name: "Federico Lara", suffix: " — líder de pensamento em Business Observability, autor de Six Steps to Achieve Business Observability, criador de Metric Monetization, e ex-executivo na New Relic e Dynatrace." },
    selectDepth: "SELECIONAR PROFUNDIDADE DO CHASSIS",
    tabs: { intro: "INTRO", introSub: "Perfil do Negócio", addis: "ADDIS", addisSub: "Camada Tecnológica", blips: "BLIPS", blipsSub: "Camada de Negócio", kbrs: "KBRs", kbrsSub: "Key Business Results" },
    expandAll: "EXPANDIR TUDO", collapseAll: "RECOLHER TUDO",
    sections: "SEÇÕES", items: "ITENS",
    businessProfile: "PERFIL DO NEGÓCIO", whyChassis: "CHASSIS", keyValuePoints: "PONTOS-CHAVE DE VALOR", frameworkApplied: "CHASSIS APLICADO",
    addisDesc: "Mapeia cada ferramenta tecnológica, plataforma, sistema e componente de infraestrutura dos quais este negócio depende.",
    blipsDesc: "Mapeia cada função de negócio e as fontes de dados observáveis que conectam o desempenho tecnológico aos resultados de negócio.",
    kbrTitle: "Key Business Results", kbrsAcross: "KBRs ACROSS", businessAreas: "BUSINESS AREAS",
    metric: "MÉTRICA", target: "META",
    newChassis: "NOVO CHASSIS",
    loadingTitle: "PROCESSANDO",
    loadingSteps: ["ANALISANDO TIPO DE NEGÓCIO...", "MAPEANDO CAMADA TECNOLÓGICA ADDIS...", "CONSTRUINDO CHASSIS DE NEGÓCIO BLIPS...", "GERANDO RESULTADOS-CHAVE DE NEGÓCIO...", "FABRICANDO CHASSIS..."],
    fabricatingPrefix: "FABRICANDO",
    charmingPhrases: [
      "Isso vai rápido — prepare-se.",
      "Ainda aqui? Ótimo. Nós também. Está indo bem.",
      "Alongue um pouco. Estaremos prontos quando você estiver.",
      "Isso está levando um momento. Um bom momento — como o de um bom espresso.",
      "Enquanto espera, vá abraçar um pet... não precisa ser o seu.",
      "Pode levar alguns minutos — peça aquela bebida complicada que você tanto ama.",
      "Estamos construindo algo personalizado, específico, seu. Boas coisas levam tempo.",
      "Continuando. Este é nosso trabalho mais completo. Vá buscar um lanche. Você merece.",
    ],
    errorTitle: "Algo deu errado.", tryAgain: "TENTAR NOVAMENTE",
    tableHeaders: ["Item", "Tipo", "Descrição", "Int/Ext", "Ambiente", "Dados"],
    tableHeadersBlips: ["Item", "Fonte", "Tipo", "Descrição", "Int/Ext", "Ambiente", "Dados"],
    noItems: "Nenhum item gerado.",
    tiers: [
      { id: "compact", label: "Compacto", description: "Visão essencial. Itens principais de ADDIS e BLIPS com KBRs focados. Ideal para avaliações rápidas." },
      { id: "midsize", label: "Médio", description: "Profundidade aprimorada com contexto mais rico. Mais itens, descrições completas e fatores do mundo real ampliados." },
      { id: "executive", label: "Executivo", description: "Análise abrangente com contexto detalhado, cobertura do mundo real e KBRs enriquecidos com benchmarks setoriais." },
      { id: "luxury", label: "Luxo", description: "Inteligência de nível empresarial. Profundidade máxima, análise narrativa de impacto, registro de riscos e vitórias rápidas." },
    ],
    examples: [
      "Häagen-Dazs, Regent Street Londres, https://www.haagen-dazs.co.uk/, Aumentar vendas 20% nos meses de inverno.",
      "Oficina de Motos, Medellín Colômbia Laureles, Adicionar serviços de aluguel e tuning personalizado, Orçamento $2.000 USD.",
      "Mercado Livre, https://www.mercadolivre.com.br/",
      "Hotel Boutique, Cartagena Colômbia, Expandir reservas de viajantes internacionais, Lançar programa de fidelidade.",
      "Tesla, https://www.tesla.com/, Otimizar operações dos centros de serviço, Reduzir tempos de espera em 30%.",
      "Farmácia Independente, Miami Flórida, Competir com grandes redes, Implementar delivery e telemedicina.",
      "Academia CrossFit, Brooklyn Nova York, Expandir base de alunos, Lançar programa de treino online.",
      "Escritório de Direito de Família, Cidade do México, Atrair clientes premium, Estratégia de presença digital.",
    ],
    beyondProfit: "BEYOND PROFIT",
    beyondProfitSub: "Opcional",
    beyondProfitDesc: "Selecione as iniciativas relevantes para este negócio. Os resultados aparecerão em uma aba dedicada.",
    beyondProfitTab: "Beyond Profit",
    beyondProfitTabSub: "Insights Jurídicos, Sociais e Regulatórios",
    bpOptions: { CSR: "RSC", ESG: "ESG", DEI: "DEI", TBL: "TBL", Sustainability: "SUS" },
    bpDefinitions: {
      CSR: "RSC — Responsabilidade Social Corporativa",
      ESG: "ESG — Ambiental, Social e Governança",
      DEI: "DEI — Diversidade, Equidade e Inclusão",
      TBL: "TBL — Triplo Resultado",
      Sustainability: "SUS — Sustentabilidade",
    },
    useTokens: "USAR",
    tokensPer: "1 TOKEN POR",
    bpLabels: { legal: "JURÍDICO", social: "SOCIAL", regulatory: "REGULATÓRIO", news: "NOTÍCIAS E DESENVOLVIMENTOS", suggestions: "SUGESTÕES" },
    bp: { analyzing: "ANALISANDO", gatheringContext: "COLETANDO CONTEXTO JURÍDICO E REGULATÓRIO...", generatingInsights: "GERANDO INSIGHTS E SUGESTÕES...", errorMsg: "Não foi possível gerar os insights Beyond Profit." },
    pdfControlled: "CONTROLADO",
    pdfUncontrolled: "NÃO CONTROLADO",
    errorEmailNotConfirmed: "Por favor confirme seu endereço de email antes de gerar. Verifique sua caixa de entrada.",
    auth: {
      signInTitle:    "Entrar no CHASS1S",
      signUpTitle:    "Criar Sua Conta",
      resetTitle:     "Redefinir Sua Senha",
      signInTab:      "ENTRAR",
      signUpTab:      "CRIAR CONTA",
      forgotTab:      "REDEFINIR",
      emailLabel:     "ENDEREÇO DE EMAIL",
      passwordLabel:  "SENHA",
      redirecting:    "REDIRECIONANDO...",
      processing:     "PROCESSANDO...",
      sending:        "ENVIANDO...",
      sendReset:      "ENVIAR EMAIL DE REDEFINIÇÃO",
      backToSignIn:   "Voltar ao Login",
      forgotLink:     "Esqueceu a senha?",
      signUpSuccess:  "Conta criada! Verifique seu email para confirmar.",
      resetSuccess:   "Email de redefinição enviado. Verifique sua caixa.",
      freeTokens:     "Novas contas recebem 10 TOKENS GRATUITOS — sem cartão de crédito.",
      orContinueWith: "OU CONTINUAR COM",
    },
    // ── Account Menu ──
    menuSignedInAs: "CONECTADO COMO",
    menuWorkspace: "ESPAÇO DE TRABALHO",
    menuPersonal: "Pessoal",
    menuAddWorkspace: "+ Espaço de Trabalho",
    menuChassis: "CHASSIS",
    menuTokenBalance: "SALDO DE TOKENS",
    menuBuyMore: "+ Comprar Mais",
    menuLanguage: "IDIOMA",
    menuSignOut: "SAIR",
    // ── Settings & Support ──
    menuSettings: "Configurações",
    menuSupport: "Suporte",
    settingsTitle: "Configurações da Conta",
    settingsSectionProfile: "INFORMAÇÕES DE PERFIL",
    settingsSave: "SALVAR ALTERAÇÕES",
    settingsSaving: "SALVANDO...",
    settingsSaved: "✓ Salvo",
    settingsSectionSecurity: "SEGURANÇA",
    settingsNewEmail: "NOVO EMAIL",
    settingsUpdateEmail: "ATUALIZAR EMAIL",
    settingsCurrentPassword: "SENHA ATUAL",
    settingsNewPassword: "NOVA SENHA",
    settingsConfirmPassword: "CONFIRMAR NOVA SENHA",
    settingsUpdatePassword: "ATUALIZAR SENHA",
    settingsOAuthNoPassword: "O login com senha não está disponível para sua conta. Você entrou com um provedor social.",
    settingsSectionConnected: "CONTAS CONECTADAS",
    settingsSectionDanger: "FECHAR CONTA",
    settingsDangerWarning: "Sua conta será agendada para exclusão em 30 dias. Você pode reativá-la fazendo login antes dessa data.",
    settingsDangerPrompt: "Digite DELETE para confirmar",
    settingsDeactivateBtn: "DESATIVAR CONTA",
    settingsDeactivated: "Conta desativada. Você será desconectado.",
    settingsDangerShow: "Mostrar opções",
    supportTitle: "Contatar Suporte",
    supportCategoryLabel: "CATEGORIA DO PROBLEMA",
    supportCategories: ["Cobrança e Tokens", "Problema Técnico", "Acesso à Conta", "Qualidade de Geração", "Solicitação de Funcionalidade", "Outro"],
    supportDescriptionLabel: "DESCRIÇÃO",
    supportDescriptionPlaceholder: "Descreva seu problema em detalhes (mínimo 20 caracteres)...",
    supportAttachLabel: "ANEXO (opcional)",
    supportChooseFile: "ESCOLHER ARQUIVO",
    supportFileSizeError: "O arquivo deve ter menos de 5MB",
    supportFileTypeError: "Apenas imagens e arquivos PDF são aceitos",
    supportSubmit: "ENVIAR TICKET",
    supportSubmitting: "ENVIANDO...",
    supportSuccessTitle: "Ticket Enviado",
    supportSuccessMsg: "Responderemos em 24–48 horas. Seu número de ticket é:",
    supportClose: "FECHAR",
    modeSelector: {
      fabricate: "Fabricar",
      tune: "Afinar",
      drive: "Conduzir",
      go: "IR",
      comingSoon: "Em Breve",
    },
    reFabricate: {
      title: "Re-Fabricar Chass1s",
      contextNote: "Seu Chass1s atual será usado para construir um resultado ainda mais rico.",
      originalInput: "ENTRADA ORIGINAL",
      editHint: "Edite ou expanda sua entrada antes de re-fabricar.",
      selectType: "SELECIONAR TIPO",
      confirm: "RE-FABRICAR",
    },
    chassisHistory: {
      title: "Histórico do Chass1s",
      actionFabricated: "Fabricado",
      actionReFabricated: "Re-Fabricado",
      actionTuned: "Afinado",
      chassisType: "TIPO DE CHASS1S",
      beyondProfit: "BEYOND PROFIT",
      none: "Nenhum",
      noHistory: "Sem histórico ainda.",
    },
  },
};

export const TIER_CONFIG = [
  { id: "compact", tokens: 64000, addisCount: "5–7", blipsCount: "4–6", kbrCount: "3–4", descLength: "1 sentence max per field", extras: "" },
  { id: "midsize", tokens: 64000, addisCount: "8–10", blipsCount: "6–8", kbrCount: "4–5", descLength: "1–2 sentences per field", extras: "Include 2–3 real-world factors per relevant BLIPS section." },
  { id: "executive", tokens: 64000, addisCount: "10–12", blipsCount: "8–10", kbrCount: "5–6", descLength: "2 sentences per field", extras: "Include 3–4 real-world factors per relevant BLIPS section. Add industry-specific benchmarks to KBR targets." },
  { id: "luxury", tokens: 64000, addisCount: "12–15", blipsCount: "10–12", kbrCount: "6–7", descLength: "2–3 sentences per field with specific business context", extras: `Include 4–5 real-world factors per relevant BLIPS section with detailed impact descriptions.\nAlso add these extra top-level JSON fields:\n"benefits": { "BizOps": "paragraph", "Logistics": "paragraph", "Inventory": "paragraph", "Production": "paragraph", "Sales": "paragraph" },\n"risks": [{ "title": "Risk title", "area": "ADDIS or BLIPS area", "severity": "High/Medium/Low", "description": "What this risk is and how Chassis detects it" }],\n"quickWins": [{ "title": "Quick win title", "description": "Specific immediate observability improvement", "impact": "Expected business impact" }]\nGenerate 5 risks and 5 quick wins.` },
];

export function getTiers(lang) {
  return TIER_CONFIG.map((cfg, i) => ({ ...cfg, ...T[lang].tiers[i] }));
}
